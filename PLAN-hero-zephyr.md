# PLAN — ZEPHYR THAY THORNQUILL + LÀM LẠI NHÁNH SUPPORT
...
> Gourd*). Khác plan một điểm, có ghi lý do tại chỗ: EDGE của Zephyr là `BONUS_DAMAGE` chứ
...
**(a) Thay hero.** Thornquill (xương rồng, RANGED) rời đội. Zephyr — phi công drone thân
Cattail — vào thay. Bảng mở khóa đổi hai ô:

| Stage | Act | Trùm | Thành phố | Hero — CŨ | Hero — MỚI |
|---|---|---|---|---|---|
| II | 1 | The Armada | Windward | Chardwall | **Zephyr** ⟵ mới |
| III | 1 | The Headliner | Neon Rose | Thornquill | **Chardwall** ⟵ dời sang |

**(b) Khung gear hai-món** (mục 4) — mỗi gear là hai đặc điểm của đúng một hero.

**(c) Nhánh SUPPORT làm lại** (mục 6): Sunspot bỏ skill sát thương lấy skill buff thuần;
Chardwall lấy skill hất-qua-đầu (push + pull); Gourdward thêm nội tại miễn nhiễm khi có giáp.

## 2 · Quyết định đã chốt

1. **Cây gốc: Cattail**, không phải Blover — Blover đã tồn tại làm item (`GUST`,
   `utils/itemResolution.ts`), hero trùng tên là đụng hàng chức năng. Prompt art đã viết theo
   Cattail (magenta, tai mèo, đạn pod).
2. **Zephyr bay thật** — `movementType: 'FLYING'`, đổi lại **máu thấp nhất roster**, và vẫn bị
   đánh bình thường khi có zombie đứng kề, bất kể đang lơ lửng trên ô gì.
3. **Gear: `MAT_CACTUS` → `MAT_CATTAIL`** — pool giữ đúng 10 gear.
4. **Đòn cơ bản của Zephyr: cặp nước-mã 4 hướng** (mục 5.2) — không phải cặp chéo kề.
5. **Kỹ năng phí của Zephyr: bom khói huỷ intent** — tái dùng cơ chế `DUST_VEIL` sẵn có (5.3).
6. **`MAT_CHOMPER` đổi trục sang chảy máu** (`BLEED_ON_HIT`) — hết trùng `RETALIATE_DAMAGE`
   với `MAT_ENDURIAN` (mục 8).
7. **Sunspot**: Solar Blessing = **+1 Shield (tồn tại xuyên lượt, CÓ CAP) + +1 damage CHỈ
   trong lượt đó** — phải buff trước rồi mới đi nước đánh. Shield phải cap vì nó sống xuyên
   lượt, và vì buff giáp là nghề chính của Gourdward — Sunspot không được giẫm. (Phương án
   thay nửa giáp: hồi 1 máu đã mất, không tăng max — cân nhắc ở 6.1.)
8. **Chardwall**: **Vault Toss là ĐÒN CƠ BẢN** (miễn phí), **ngã 1 sát thương**; zombie đáp
   đất **dính nguyên tố của anh** — đúng chữ luật L1 sẵn có (*"a Chardwall shove now lands a
...
10. **NHÀ: mô hình "unit 1 máu" là ĐÚNG — và engine đã nghĩ đúng như thế sẵn rồi.** Zombie
    ...
    stood beside the house for a full turn first"*, *"under ITB's rule… still standing
...
11. **Sunspot mang nguyên tố = CỤC PIN NGUYÊN TỐ.** Miễn nhiễm nguyên tố giữ cho bản thân
...
14. **GIÁP = LỚP CHẮN kiểu ItB, toàn hệ thống** (mục 6.0): một lớp chặn TRỌN một nguồn sát
    ...
    MỘT lớp.
15. **Lớp vs các kiểu đòn**: **Devour của Maw KHÔNG xé được lớp** — một cú nuốt là MỘT nguồn,
...
|---|---|---|
| `FLYING` đầy đủ (địa hình, thân thể, chết đuối) | `gameLogic.ts:422,453,101` | Zephyr |
...
| **`BUFF_STAT` phân giải được** (`stat: 'HP' \| 'DMG'`), và `getValidSkillTargets` **đã nhận nó là skill nhắm đồng minh** | `skillResolution.ts:206`, `gameLogic.ts:710`; bench Cactus có skill *Stretch* dùng nó | Sunspot buff |
| **`PULL` phân giải được**, `PUSH_DISTANCE` cộng cho cả PULL | `skillResolution.ts:436-456`, `fusion.ts:364` | Chardwall |
| Skill nhắm đồng minh trả phí — tiền lệ nguyên vẹn | Encase của Gourdward (`SHIELD`, LOB 3) | Sunspot buff |
| Miễn nhiễm bẩm sinh trên hero | Sunspot `immunities: ['BURN']` (`heroes.ts:121`) | Gourdward ward |
| Từ vựng miễn nhiễm `BURN / FREEZE / SHOCK` | `types.ts:152` | Gourdward ward |
| Sát thương va chạm + `COLLISION_BONUS` | `planPush`, recipe *Grand Chard* | Cú hất của Chardwall |
| Blover item lọc `isEnemy` → không thổi hero bay phe mình | `itemResolution.ts:230` | Zephyr |
| **Ăn não ĐÃ là đòn telegraph**: zombie đứng kề 1 lượt, intent `ATTACK` nhắm nhà, cắn ở lượt sau và Ở LẠI | `turnManager.ts:697-717`, `aiLogic.ts:13` | Giáp NHÀ — chỉ cần chặn đúng cú cắn |
| Đường đẩy-vào-nhà lấy não (cửa thứ hai của não) | `gameLogic.ts:194` | Giáp NHÀ — điểm cắm thứ hai |

### 3.2 Bẫy đã phát hiện

1. **`roster.assert.ts` bắt đúng 3 RANGED / 3 MELEE / 3 SUPPORT, tổng 9.** Zephyr bắt buộc
   `RANGED` (Thornquill cũng vậy).
2. **`RADIUS` là code chết** — có trong `SkillRangeType`, không skill nào dùng, không chỗ nào
   phân giải. Không dùng cho Zephyr (đúng vết xe `DROWN` mà repo tự cảnh báo).
3. **Thornquill là mốc cân bằng được viện dẫn ở 5 comment**: `bossBehaviours.ts:769` (trùm mất
   ...
   đổi luật.**
4. **Quỹ đạo đạn RANGED đã kín chỗ** — Shadeleaf `LINE`+`VOLLEY`, Cobb `LOB` (comment của Cobb
   nói thẳng "mọi đòn khác là LINE"). → Zephyr khác biệt ở **di chuyển + hình học nước mã**,
...
6. **Chi phí lớn nhất của cả plan: kịch bản tutorial.** Sun Burn được script dùng ở **4 bàn**
   với số học chính xác (xem 6.1). Đổi skill Sunspot = viết lại các bàn đó + assert của chúng.

### 3.3 Chi phí ma trận fusion

`FUSION_RECIPES` là `Record<HeroId, Record<MaterialId, …>>` đầy đủ — thiếu ô là lỗi biên dịch.
Khối lượng: 10 công thức Zephyr + 8 công thức cột `MAT_CATTAIL` + 4 công thức đổi theo trục
...
|---|---|---|---|
| Peashooter | Shadeleaf · *Pea Shot / Precision Blast* | bắn thẳng (`GRANT_ATTACK` cho ai không có) | `DOUBLE_ATTACK` |
| Corn | Cobb · *Corn Kernel / Butter Splat* | `ARC_ATTACK` | `SKILL_STUN` *(mới)* — kỹ năng phí được ghép STUN. Hợp lệ với STUN RULE: stun trả phí là "the paid, honest shape" |
| **Cattail** | **Zephyr** · *Wing Guns / Smoke Pod* | `MOVE_BONUS` *(mới)* | `SKILL_DISARM` *(mới)* — kỹ năng phí để lại bụi tước đòn |
| Wall-nut | Ironhusk · *Shield Bash / Rolling Charge* | `BONUS_HP` | `DAMAGE_REDUCTION` |
| Chomper | Maw · *Bite / Devour* | `BONUS_DAMAGE` | `BLEED_ON_HIT` *(mới)* |
| Endurian | Thornhide · *Thorn Swipe / Provoke* | `RETALIATE_DAMAGE` | `TAUNT_RADIUS` (chỉ nghĩa với ai có taunt → các ô khác thường rơi về A hoặc ngoại lệ) |
| Sunflower | Sunspot · *Harvest / **Solar Blessing*** | `SUN_PER_TURN` · `SUN_ON_KILL` | `SKILL_BLESS` *(mới)* — kỹ năng phí xong còn ban +1 damage (lượt này) cho một đồng minh gần |
| Chard | Chardwall · ***Vault Toss / Sweep*** | `PUSH_DISTANCE` (đã cộng cho cả PULL sẵn; KHÔNG kéo giãn toss — điểm rơi toss là hình học cố định) | `SKILL_DISPLACE` *(mới)* — kỹ năng phí còn dời mục tiêu 1 ô, **đẩy ra hoặc kéo về, chọn lúc ngắm** |
| Pumpkin | Gourdward · ***Reinforce / Encase*** | tự-giáp: `SHIELD_ON_KILL` (đã có, có cap) | `ELEMENT_WARD` *(mới)* — **miễn BURN + FREEZE + SHOCK**, áp thẳng vào `immunities` lúc fuse (đường ELEMENT_IMMUNITY sẵn có, zero refactor) |
| Snow Pea | ⚠️ **KHÔNG CHỦ** (mục 9) | `ON_HIT_SLOW` | `ON_HIT_FREEZE` |
...
```
role: RANGED            baseClass: UnitClass.CATTAIL (thêm mới)
...
```
        . X . X .          Hướng LÊN, Zephyr ở Z: (x-2, y-1) và (x-2, y+1)
        . . . . .
...
- Hai ô cách nhau đúng 2 ô → **ăn cả hai phát là bài toán đội hình**; bay + move 4 là công cụ
  giải bài toán đó. Không ô nào kề Zephyr → cô luôn bắn từ ngoài tầm với của thứ cô bắn.
...
### 5.3 Kỹ năng phí — `Smoke Pod` (50 Sun)
...
### 5.4 Dòng fusion của Zephyr — 10 công thức
...
|---|---|---|---|
| Sunflower | **Solar Rotor** | `SUN_ON_KILL 15` | Hai súng = hai cơ hội kết liễu mỗi lượt |
| Peashooter | **Twin Pods** | `DOUBLE_ATTACK 1` | Loạt thứ hai 1 sát thương — đúng khuôn đã hạ của Shadeleaf |
| Chomper | **Grinder Pods** | `BLEED_ON_HIT` | Hai súng = **hai vết thương mỗi lượt** cho cả đội thu hoạch |
| Wall-nut | **Armored Fuselage** | `BONUS_HP 3` | 4 → 7. Mua thẳng vào điểm yếu lõi |
| Corn | **Cluster Load** | `SKILL_DISCOUNT 15` | NGOẠI LỆ: arc vô nghĩa với hình học cố định, `SKILL_STUN` ghép vào bom diện rộng thì quá mạnh → mua rẻ quả khói (đúng ngoại lệ Cobb×Cactus cũ) |
| **Cattail** | **Overdrive Rotor** | `MOVE_BONUS 1` | Ô chữ ký: move 4 → 5, bay. Chính cô, vặn to lên |
| Endurian | **Barbed Skids** | `RETALIATE_PUSH` | Với hero 4 máu, đẩy kẻ chạm vào ra xa **là** đường thoát |
| Chard | **Downwash** | `ON_HIT_PUSH` | Gió cánh quạt: đẩy **cả hai ô** cùng lúc — chưa ai làm được |
| Pumpkin | **Pod Plating** | `SHIELD_ON_KILL` | Kết liễu là dựng lớp chắn (6.0) — hero 4 máu tự mua bảo hiểm bằng chính hai khẩu súng |

*(9 công thức — cột Snow Pea không còn tồn tại, quyết định 12.)*

## 6 · NHÁNH SUPPORT LÀM LẠI

> Ba hero support hiện chỉ trao *chỉ số*; ba gear của họ là ba ô "chưa có gì" trong khung.
> Sau mục này, ba support **nuôi lẫn nhau**: Sunspot phát giáp+damage, Gourdward biến giáp
> thành miễn nhiễm, Chardwall quyết định vị trí — và mỗi gear có món B thật.
...
| Giá trị | Cố định ≤ số giáp | **Co giãn theo đòn nó chặn**: chặn nắm đấm 5 của Gargantuar = 5, chặn imp cắn 1 = 1 → kỹ năng nằm ở việc ĐỌC đòn to mà che, không ở việc đắp số |
...
| Nhà | "não 1 máu + giáp N" — vẫn phải đếm | **"nhà có 1 lớp = đỡ đúng 1 cú cắn"** — khớp hoàn hảo mô hình unit-1-máu đã chốt |
...
| Sunspot vs Gourdward | Phân vai bằng SỐ (cô vá 1-2, anh đắp 5) | Phân vai bằng ĐỘ PHỦ: ai cũng ban "một lớp" — nhưng Gourdward ban **diện rộng + cho cả NHÀ + miễn phí mỗi lượt**, Sunspot chỉ ban kèm trong gói buff. Vai anh là coverage, vai cô là tempo |
...
2. **Ngữ nghĩa pierce — ĐÃ CHỐT (quyết định 15)**: một nguồn = chặn trọn (Devour KHÔNG xé
   ...
   Shadeleaf, phe zombie phá lớp bằng búa đôi của Clockjaw.
3. **Encase "2 mỗi người" quy về "mỗi người MỘT LỚP"** — giá trị của skill giờ nằm ở bề rộng
   dấu cộng, không ở con số. `ARMOR_WHILE_DIGESTING` của Maw → "bắt đầu tiêu hoá là có lớp"
...
### 6.1 Sunspot — Sun Burn → `Solar Blessing` (buff thuần)

**Hiện tại:** Sun Burn — LOB 3, 50 Sun, `DAMAGE 4`. Là **nguồn sát thương duy nhất** của cô.

**Mới:** `Solar Blessing` — LOB 3, 50 Sun, nhắm đồng minh:
...
- **+1 damage CHỈ TRONG LƯỢT NÀY** — status mới `BLESSED`, **xoá ở cuối lượt người chơi**
  ...
  cho ai đã đánh xong là ném 50 Sun đi.
...
1. **Tutorial script 4 bàn dùng Sun Burn với số học chính xác** (`data/tutorial.ts:562, 618,
   736, 749, 980, 1018`): bàn hộp thư ("một Sun Burn thiếu 25"), bàn cửa (Sun Burn 3 xuyên
   giáp lượt 1), bàn trùm (pea 2 + blast 3 + pea 2 + 2 bash + **Sun Burn 4 = 13 vào 16 HP** —
   "Measured, so the defeat can never be mistaken for a misplay"). Re-script + assert. Bàn
   trùm còn **hay hơn về truyện**: cô không hại nổi Gargantuar, cô ban phước cho Ironhusk đập
   nó — và +1-trong-lượt dạy đúng bài *buff trước, đánh sau* ngay tại đó.
2. Recipe vỡ: *Winter Flare* ("Sun Burn slows…") và *Needle Bloom* (`SKILL_SPLASH` trên Sun
...
3. Comment định giá Devour theo "TWO Sun Burns (8 damage…)" (`heroes.ts:176`) — neo lại.
4. Element rider trên skill nhắm đồng minh: tiền lệ Encase đã giải — kiểm một lần khi làm.

**Nguyên tố trên Sunspot — CỤC PIN NGUYÊN TỐ (quyết định 11).** Cô giữ miễn nhiễm của
...
Fire-Sunspot bless Maw đang đứng cạnh ba xác → Bite rực lửa. ICE-Sunspot bless Shadeleaf →
volley 3 viên đạn băng. Đây là fantasy support mà nhánh này còn thiếu.

Gear Sunflower món B: `SKILL_BLESS` — *kỹ năng phí của bạn xong còn ban +1 damage (lượt này)
...
### 6.2 Chardwall — Vault Toss là ĐÒN CƠ BẢN, Sweep giữ nguyên làm kỹ năng
...
- **Kỹ năng `Sweep`** (50 Sun, SELF radial `PUSH 2`) — **giữ nguyên như đang có.** "Đẩy lùi 4
...
- Va chạm/môi trường **bỏ qua giáp mũ** theo luật hiện hành — Vault Toss là câu-trả-lời-to cho
  Buckethead mà game vẫn luôn đòi ("push it, burn it, spike its path").
...
- **Passive:** `immunities: ['BURN', 'FREEZE', 'SHOCK']` — **vô điều kiện, tĩnh**. Đúng mạch
  ...
  ba. Tiền lệ: Sunspot `['BURN']`. **Chi phí engine: BẰNG KHÔNG** — chỉ là phần tử mảng, cả
...
- **Kỹ năng `Encase`** (50 Sun): **AoE hình dấu cộng** — mọi đồng minh trên ô anh đứng + 4 ô
...
- **Hai điểm chặn**, đúng hai cửa não bị lấy: cú cắn kề (`turnManager.ts:711`) và cú bị-đẩy-
  vào-nhà (`gameLogic.ts:194` — cửa này tồn tại làm giá của đòn đẩy ẩu, lớp cũng phải đỡ được
  nó). Có lớp → lớp vỡ, não KHÔNG mất, hiện icon vỡ; zombie vẫn đứng đó cắn tiếp — Reinforce
  lượt sau là cuộc giằng co 1-đổi-1 giữa Gourdward và con zombie đó, đúng nhịp tug-of-war ItB.
- `Tile.tsx` vẽ lớp trên nhà · `threat.ts` coi nhà-còn-lớp không phải "não sắp mất".
...
**Nguyên tố trên support:** Sunspot đã có lối thoát đẹp — cục pin cho mượn qua Blessing (6.1).
...
(Chardwall thoát theo đúng chữ L1: cú hất 0-damage vẫn chở nguyên tố — giờ thành chốt ở cú
đáp đất của Vault Toss.)

Tam giác support sau chốt: **Sunspot vá giáp lặt vặt + châm damage đúng lượt → Gourdward là
nguồn giáp chính và là người duy nhất che được NHÀ → Chardwall quyết định kẻ địch đứng đâu.**
Ba người, không ai có một đòn sát thương trực tiếp — nhánh support là nhánh *enabler* thuần.

Gear Pumpkin: món A = tự-giáp `SHIELD_ON_KILL` (có cap, tiền lệ *Gourd Husk*); món B =
...
| Hero | Món | Tên | Vì sao |
|---|---|---|---|
| Sunspot | `SKILL_DISARM` | **Ashveil** | Blessing kèm bụi quanh người được ban phước — che luôn kẻ được che |
| Shadeleaf | `SKILL_DISARM` | **Smokeline** | Precision Blast quét hàng nào, hàng đó không vung nổi đòn |
| Maw | `MOVE_BONUS 1` | **Prowl Drive** | Phải áp sát mới cắn, tiêu hoá xong lại kẹt tại chỗ |
| Ironhusk | `MOVE_BONUS 1` | **Quick Bulwark** | Đến kịp hành lang cần chặn là toàn bộ việc của cô |
| Cobb | `MOVE_BONUS 1` | **Skid Carriage** | LOB 2 buộc đứng sát tuyến; thêm chân là thêm đường lui |
| Thornhide | `MOVE_BONUS 1` | **Windburr** | Chính chữ trong file: *"2 damage and move 2 catches nobody"* — cho anh chọn chỗ đặt bẫy khiêu khích |
| Chardwall | `SKILL_DISARM` | **Veilsweep** | Sweep: bốn ô vừa quét chìm trong bụi — bị hất văng xong còn không vung nổi đòn |
...
2`), và 3/9 hero nhận đúng phản đòn từ Chomper — chiếm trục của Endurian.
...
| Cobb | *Cob Grinder* `RETALIATE_DAMAGE 2` | **Shrapnel Kernel** `BLEED_ON_HIT` |
| Chardwall | *Snapping Guard* `RETALIATE_DAMAGE 2` | **Rending Guard** `BLEED_ON_HIT` — ô đáng nhất: hero 0-damage giờ **đánh dấu để cả đội giết mạnh hơn**, lọt qua guard cấm-damage của anh một cách chính danh |
| Zephyr | *(mới)* | **Grinder Pods** `BLEED_ON_HIT` |
| `materials.ts` | `RETALIATE_DAMAGE 2` | `BLEED_ON_HIT` — hết trùng Endurian |

## 9 · `MAT_SNOW_PEA` — BỎ HẲN (đã chốt, quyết định 12)

Khung hai-món làm lộ: Snow Pea không thuộc hero nào — Frostpod đã nghỉ, cái lạnh giờ là nguyên
...
- Item **Snow Pea** (`items.ts`, hiệu ứng FREEZE) là thứ KHÁC, không đụng — chỉ gear chết.
...
1. `art-src/ART-PROMPTS-CUTSCENES.md` §2.9 — prompt Headliner đang tả *Thornquill cầm giáo
   xuyên hàng* → đổi thành **Chardwall hất bay đám đông** (0 damage, đẩy — giờ thêm Vault Toss)
2. `data/cutscenes.ts` `DISCO_ZOMBOSS` — caption *"One spine ran through all of them at once"*
   → viết lại theo Chardwall
3. `data/cutscenes.ts` `BALLOON_ARMADA` — caption *"took back the hands that threw it"* (viết
   cho Chardwall) → viết lại theo Zephyr chiếm không trung
...
| File | Việc |
|---|---|
| `types.ts` | `HeroId` −THORNQUILL +ZEPHYR · `UnitClass` +CATTAIL · `MaterialId` MAT_CACTUS→MAT_CATTAIL · `SkillRangeType` +`WING_PAIR` · `EffectType` +`DUST_TILE` +`TOSS` · `FusionEffectType` +`MOVE_BONUS` +`SKILL_DISARM` +`SKILL_STUN` +`SKILL_BLESS` +`SKILL_DISPLACE` +`ELEMENT_WARD` +`BLEED_ON_HIT` · `StatusEffectType` +`BLEEDING` +`BLESSED` · **`TileData.shield?`** (giáp NHÀ) |
| `data/heroes.ts` | Khối THORNQUILL → ZEPHYR (5.1–5.3) · Sunspot: Sun Burn → Solar Blessing · Chardwall: Backswing → Vault Toss (đòn cơ bản), giữ Sweep · Gourdward: Rind Bash → Reinforce, Encase → AoE dấu cộng, `immunities: ['BURN','FREEZE','SHOCK']`, `damage: 0` |
...
| `data/heroUpgrades.ts` | EDGE THORNQUILL → ZEPHYR (gợi ý `MOVE_BONUS`) · EDGE GOURDWARD *Hard Rind* (BONUS_DAMAGE) chết theo Rind Bash → đổi (gợi ý: Reinforce +1) · *Long Handle* của Chardwall giữ, sửa mô tả thành tầm túm |
| `data/unlocks.ts` | 2 dòng BOSSES (hero Armada + Headliner) · `HERO_MATERIAL` · viết lại `hint` hai trùm theo hero mới |
| `data/tutorial.ts` + `tutorial.assert.ts` | **Re-script các bàn dùng Sun Burn** (6.1) — mục đắt nhất |
...
| `utils/{bossBehaviours,fusion}.ts` | Comment mốc Thornquill (3.2-3) · guard fusion cho kit mới của Chardwall/Sunspot/Gourdward |
| `utils/threat.ts` | Nhà có giáp không còn báo "não sắp mất" trong overlay dự báo |
...
| `i18n/vi.ts` | Zephyr + 4 skill mới (Blessing/Toss/Reinforce/Encase-AoE) + ~26 recipe + upgrade + 2 hint + caption comic |
...
2. **Zephyr thay Thornquill (5)**: types, hero, `WING_PAIR`, `DUST_TILE`/Smoke Pod, gear
   `MAT_CATTAIL` + cột của nó, dòng fusion Zephyr, bỏ `MAT_SNOW_PEA` (9), trục chảy máu của
...
4. **Tutorial re-script (6.1.1)**: các bàn dùng Sun Burn — làm CUỐI vì phụ thuộc kit Sunspot
...
- Chơi thử: bàn có nước/dung nham cho `FLYING` (hero bay đầu tiên phe người chơi) · trận
  ...
  Reinforce lên NHÀ rồi để zombie tới cắn — não phải còn, lớp phải vỡ, lượt sau nó cắn tiếp ·
