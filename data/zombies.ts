...
export const ZOMBIE_DEFINITIONS: Partial<Record<UnitClass, UnitDefinition>> = {
    ...
    [UnitClass.BUCKETHEAD]: {
        class: UnitClass.BUCKETHEAD, name: 'Buckethead', maxHp: 3, damage: 2, moveRange: 3, armor: 1,
     ...
     * sentence already written above the Flag Zombie, turned into a boss.
     *
     ...
     * five, so more than half of every boss fight in the game deleted Ironhusk, Chardwall and
     * the whole ICE element — and Chardwall deals zero damage, which made him a spectator in
     ...
     *
     *   Gargantuar       PUSH     the act-one lesson: the first thing too big to move
     ...
     * one-tile puzzle on a Screen Door zombie and far too blunt across a whole boss fight.
     *
     ...
     * needed at all because SLAY_BOSS no longer runs on a clock: Sun accrues every turn
     * forever, so a stun every turn would otherwise lock the final boss out of the game.
     */
    [UnitClass.DISCO_ZOMBOSS]: {
        class: UnitClass.DISCO_ZOMBOSS, name: 'The Headliner', maxHp: 20, damage: 1, moveRange: 3,
     ...
     *
     * 24 HP: a five-turn body against the Gargantuar's three and the Colossus's four — the
     ...
     *
     * 3 damage reads small next to the Gargantuar's 5 — until it doubles against something
     ...
     * the same figure Voltmaw puts on a clumped squad and one more than a Gargantuar's swing.
     *
     ...
     * lands, which is the real counterplay: dragged onto Thornhide, its own second hand
     ...
     * UnitClass.GARGANTUAR — which meant the final boss of the campaign was literally being
     * fought as a Gargantuar, wearing its art and its 16 HP.
     *
     * NOT in MASSIVE_BOSSES, deliberately. `isMassive` also blocks being eaten and frozen, so
     * it would delete Ironhusk and Chardwall from the last fight in the game. The immunities
     * below say exactly what it resists and nothing more; Maw's Devour is already held off by
     ...
     * has to be: the eruption is a ring you want broken up, and Chardwall was the reward for the
